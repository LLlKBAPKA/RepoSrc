...
public abstract class LivingEntity extends Entity {
    ...
    private final Animation healthAnim = new Animation(), absorbtionAnim = new Animation();
    ...
    protected void updateFallState(double y, boolean onGroundIn, BlockState state, BlockPos pos) {
        ...
        if (!this.world.isRemote && this.fallDistance > 3.0F && onGroundIn) {
            ...
            if (!state.isAir()) {
                ...
                int i = (int) (150.0D * d0);
                ((ServerWorld) this.world).spawnParticle(new BlockParticleData(ParticleTypes.BLOCK, state), this.getPosX(), this.getPosY(), this.getPosZ(), i, 0.0D, 0.0D, 0.0D, 0.15F);
            }
        }
    ...
    protected void func_233642_cO_() {
        if (!this.getStateBelow().isAir()) {
            ...
            if (i > 0 && this.func_230296_cM_()) {
                ...
                modifiableattributeinstance.applyNonPersistentModifier(new AttributeModifier(SOUL_SPEED_BOOT_ID, "Soul speed boost", 0.03F * (1.0F + (float) i * 0.35F), AttributeModifier.Operation.ADDITION));
    ...
    public boolean isPotionActive(Effect potionIn) {
        ...
        if (noRender.isEnabled() && noRender.elements().getValue("Плохие эффекты") && (potionIn == Effects.NAUSEA || potionIn == Effects.BLINDNESS))
            return false;
    ...
    public float getHealthFixed() {
        float defaultHealth = getMaxHealth();
        float health = getHealth();

        if (PlayerUtil.isFuntime()) {
            health = getFuntimeHealth(health);
        } else if (PlayerUtil.isBedwars()) {
            health = getBedwarsHealth(health);
        }

        if (isAlive() && (Float.isInfinite(health) || Float.isNaN(health) || health == Integer.MAX_VALUE)) {
    ...
    private float getFuntimeHealth(float currentHealth) {
        AtomicReference<Float> hp = new AtomicReference<>(currentHealth);
        Scoreboard scoreboard = world.getScoreboard();
        ScoreObjective objective = scoreboard.getObjectiveInDisplaySlot(2);

        if (objective != null) {
            scoreboard.getObjectivesForEntity(getScoreboardName()).entrySet().stream()
                    .filter(entry -> entry.getValue().getObjective().getDisplayName().getString().contains("Здоровья"))
                    .findFirst()
                    .ifPresent(entry -> hp.set((float) entry.getValue().getScorePoints()));
        }

        return hp.get() != null ? hp.get() : this.isInvisible() ? getMaxHealth() : currentHealth;
    }

    private float getBedwarsHealth(float currentHealth) {
        Scoreboard scoreboard = world.getScoreboard();

        for (Map.Entry<ScoreObjective, Score> entry : scoreboard.getObjectivesForEntity(getScoreboardName()).entrySet()) {
            if (entry.getKey() == null) {
                continue;
            }
            if ("tabhealthbar".equals(entry.getKey().getDisplayName().getString())) {
                return (float) entry.getValue().getScorePoints();
            }
        }
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        if (this.isInvulnerableTo(source)) {
            ...
            if (entity1 != null) {
                ...
                } else if (entity1 instanceof WolfEntity wolfentity) {
    ...
    private boolean canGoThroughtTrapDoorOnLadder(BlockPos pos, BlockState state) {
        if (state.get(TrapDoorBlock.OPEN)) {
            ...
            return blockstate.isIn(Blocks.LADDER) && blockstate.get(LadderBlock.FACING) == state.get(TrapDoorBlock.HORIZONTAL_FACING);
        }
    ...
    private int getArmSwingAnimationEnd() {
        ...
        SwingAnimationEvent event = new SwingAnimationEvent(animation);
        event.hook();
        animation = event.getAnimation();

        return (int) (animation * Minecraft.getInstance().timer.getSpeed());
    }
    ...
    protected void jump() {
        ...
        if (this.isPotionActive(Effects.JUMP_BOOST)) {
            jumpMotion += 0.1F * (float) (this.getActivePotionEffect(Effects.JUMP_BOOST).getAmplifier() + 1);
        }

        float yaw = rotationYaw;

        if (this instanceof ClientPlayerEntity) {
            JumpEvent event = new JumpEvent(jumpMotion, yaw);
            event.hook();
        ...
        Vector3d vector3d = this.getMotion();
        this.setMotion(vector3d.x, jumpMotion, vector3d.z);

        if (this.isSprinting()) {
            float f1 = yaw * ((float) Math.PI / 180F);
            this.setMotion(this.getMotion().add(-MathHelper.sin(f1) * 0.2F, 0, MathHelper.cos(f1) * 0.2F));
        }
    ...
    protected void handleFluidSneak() {
        this.setMotion(this.getMotion().add(0.0D, -0.04F, 0.0D));
    }
    ...
    public boolean canEntityBeSeen(Vector3d pos) {
        Vector3d vector3d = new Vector3d(this.getPosX(), this.getPosYEye(), this.getPosZ());
        Vector3d vector3d1 = new Vector3d(pos.x, pos.y, pos.z);
    ...
    public boolean canBePushed() {
        ...
        if (noPush.isEnabled() && noPush.checks().getValue("Игроков") && this instanceof ClientPlayerEntity) {
            return false;
        }
    ...
    public float getSmoothAbsorptionAmount() {
        return (float) Mathf.round(absorbtionAnim.getValue(), 1);
    }
    ...
    public boolean shouldTriggerItemUseEffects() {
    ...
    public void updateSwimAnimation() {
    ...
    private boolean isInValidBed() {
        return this.getBedPosition().map((p_241350_1_) ->
                this.world.getBlockState(p_241350_1_).getBlock() instanceof BedBlock).orElse(false);
    }
