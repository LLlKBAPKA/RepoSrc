...
public class PlayerInventory implements IInventory, INameable {
    ...
    /**
     * Decrement the number of animations remaining. Only called on client side. This is used to post the animation of
     * receiving a block.
     */
    public void tick() {
    ...
    public float getDestroySpeed(BlockState state) {
        if ((AutoTool.getInstance().isEnabled() && AutoTool.getInstance().status())) {
            return AutoTool.getInstance().itemSlot().getStack().getDestroySpeed(state);
        }
