...
import org.excellent.client.managers.module.impl.player.AutoTool;
...
public abstract class PlayerEntity extends LivingEntity {
    ...
    public PlayerEntity(World p_i241920_1_, BlockPos p_i241920_2_, float p_i241920_3_, GameProfile p_i241920_4_) {
        ...
        this.setLocationAndAngles((double) p_i241920_2_.getX() + 0.5D, p_i241920_2_.getY() + 1, (double) p_i241920_2_.getZ() + 0.5D, p_i241920_3_, 0.0F);
    ...
    public static AttributeModifierMap.MutableAttribute func_234570_el_() {
        return LivingEntity.registerAttributes().createMutableAttribute(Attributes.ATTACK_DAMAGE, 1.0D).createMutableAttribute(Attributes.MOVEMENT_SPEED, 0.1F).createMutableAttribute(Attributes.ATTACK_SPEED).createMutableAttribute(Attributes.LUCK);
    }
     ...
     */
    public void tick() {
        this.noClip = this.isSpectator() || FreeCam.getInstance().isEnabled();
    ...
    public void onDeath(DamageSource cause) {
        ...
        if (cause != null) {
            this.setMotion(-MathHelper.cos((this.attackedAtYaw + this.rotationYaw) * ((float) Math.PI / 180F)) * 0.1F, 0.1F, -MathHelper.sin((this.attackedAtYaw + this.rotationYaw) * ((float) Math.PI / 180F)) * 0.1F);
        } else {
    ...
    public float getDigSpeed(BlockState state) {
        ...
        if (f > 1.0F) {
            ...
            ItemStack itemstack = (AutoTool.getInstance().isEnabled() && AutoTool.getInstance().status() ? AutoTool.getInstance().itemSlot().getStack() : this.itemStackMainHand);
    ...
    public boolean func_234569_d_(BlockState p_234569_1_) {
        if (AutoTool.getInstance().isEnabled() && AutoTool.getInstance().status()) {
            return true;
        }
        return !p_234569_1_.getRequiresTool() || this.inventory.getStackInSlot(this.inventory.currentItem).canHarvestBlock(p_234569_1_);
    }
    ...
    protected Vector3d maybeBackOffFromEdge(Vector3d vec, MoverType mover) {
        if (!this.abilities.isFlying && (mover == MoverType.SELF || mover == MoverType.PLAYER) && this.isStayingOnGroundSurface() && this.func_242375_q()) {
            ...
            while (d0 != 0.0D && this.world.hasNoCollisions(this, this.getBoundingBox().offset(d0, -this.stepHeight, 0.0D))) {
            ...
            while (d1 != 0.0D && this.world.hasNoCollisions(this, this.getBoundingBox().offset(0.0D, -this.stepHeight, d1))) {
            ...
            while (d0 != 0.0D && d1 != 0.0D && this.world.hasNoCollisions(this, this.getBoundingBox().offset(d0, -this.stepHeight, d1))) {
                if (d0 < 0.05D && d0 >= -0.05D) {
                    d0 = 0.0D;
                } else if (d0 > 0.0D) {
                    d0 -= 0.05D;
    ...
    private boolean func_242375_q() {
        return this.onGround || this.fallDistance < this.stepHeight && !this.world.hasNoCollisions(this, this.getBoundingBox().offset(0.0D, this.fallDistance - this.stepHeight, 0.0D));
    }
            ...
            if (!targetEntity.hitByEntity(this)) {
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (flag5) {
                        if (i > 0) {
                            if (targetEntity instanceof LivingEntity) {
                                ((LivingEntity) targetEntity).applyKnockback((float) i * 0.5F, MathHelper.sin(this.rotationYaw * ((float) Math.PI / 180F)), -MathHelper.cos(this.rotationYaw * ((float) Math.PI / 180F)));
                            } else {
                                targetEntity.addVelocity(-MathHelper.sin(this.rotationYaw * ((float) Math.PI / 180F)) * (float) i * 0.5F, 0.1D, MathHelper.cos(this.rotationYaw * ((float) Math.PI / 180F)) * (float) i * 0.5F);
                            }
                        ...
                        if (flag3) {
                            ...
                            for (LivingEntity livingentity : this.world.getEntitiesWithinAABB(LivingEntity.class, targetEntity.getBoundingBox().grow(1.0D, 0.25D, 1.0D))) {
                                if (livingentity != this && livingentity != targetEntity && !this.isOnSameTeam(livingentity) && (!(livingentity instanceof ArmorStandEntity) || !((ArmorStandEntity) livingentity).hasMarker()) && this.getDistanceSq(livingentity) < 9.0D) {
                                    livingentity.applyKnockback(0.4F, MathHelper.sin(this.rotationYaw * ((float) Math.PI / 180F)), -MathHelper.cos(this.rotationYaw * ((float) Math.PI / 180F)));
    ...
    public boolean isPushedByWater() {
        ...
        if (noPush.isEnabled() && noPush.checks().getValue("Воды") && this instanceof ClientPlayerEntity) {
            return false;
        }
        return !this.abilities.isFlying;
    }
    ...
    public Vector3d getLeashPosition(float partialTicks) {
        ...
        if (!this.isElytraFlying() && !this.isSpinAttacking()) {
            if (this.isActualySwimming()) {
                return this.getInterpolatedPosition(partialTicks).add((new Vector3d(d0, 0.2D, -0.15D)).rotatePitch(-f).rotateYaw(-f1));
            } else {
                double d5 = this.getBoundingBox().getYSize() - 1.0D;
                double d6 = this.isCrouching() ? -0.2D : 0.07D;
                return this.getInterpolatedPosition(partialTicks).add((new Vector3d(d0, d5, d6)).rotateYaw(-f1));
            }
        } else {
            ...
            return this.getInterpolatedPosition(partialTicks).add((new Vector3d(d0, -0.11D, 0.85D)).rotateRoll(-f2).rotatePitch(-f).rotateYaw(-f1));
        }
    }
