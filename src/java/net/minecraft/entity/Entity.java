...
import org.excellent.client.managers.events.player.PostMoveEvent;
import org.excellent.client.managers.events.player.SpeedFactorEvent;
import org.excellent.client.managers.events.player.StrafeEvent;
import org.excellent.client.managers.module.impl.misc.SeeInvisibles;
...
public abstract class Entity implements INameable, ICommandSource {
    ...
    public float prevFallDistance, fallDistance;
    ...
    protected final Random rand = new FastRandom();
    ...
    public boolean canBeRaytracing() {
        EntityRayTraceEvent event = new EntityRayTraceEvent(this);
        event.hook();
    ...
    public void move(MoverType typeIn, Vector3d pos) {
        ...
        if (this.noClip) {
            ...
            if (this instanceof ClientPlayerEntity) {
                ...
                MoveEvent moveEvent = new MoveEvent(fromPosition, predictedPosition, pos, onGround, bb, ignoreHorizontal, ignoreVertical, collidedHorizontally, collidedVertically);
                moveEvent.hook();

                pos = moveEvent.getMotion();
                ignoreHorizontal = moveEvent.isIgnoreHorizontal();
                ignoreVertical = moveEvent.isIgnoreVertical();
            ...
            if (this instanceof ClientPlayerEntity) {
                ...
                PostMoveEvent postMoveEvent = new PostMoveEvent(Math.sqrt(deltaX * deltaX + deltaZ * deltaZ));
                postMoveEvent.hook();
            }
            ...
            this.setMotion(this.getMotion().mul(f2, 1.0D, f2));

            if (this.world.getStatesInArea(this.getBoundingBox().shrink(0.001D)).noneMatch((p_233572_0_) -> p_233572_0_.isIn(BlockTags.FIRE) || p_233572_0_.isIn(Blocks.LAVA)) && this.fire <= 0) {
    ...
    protected float getSpeedFactor() {
        ...
        if (block != Blocks.WATER && block != Blocks.BUBBLE_COLUMN) {
            speed = (double) f == 1.0D ? this.world.getBlockState(this.getPositionUnderneath()).getBlock().getSpeedFactor() : f;
        } else {
            speed = f;
        }

        SpeedFactorEvent event = new SpeedFactorEvent(speed);

        if (this instanceof ClientPlayerEntity) event.hook();
    ...
    private Vector3d getAllowedMovement(Vector3d vec, boolean horizontal, boolean vertical) {
        ...
        if (this.stepHeight > 0.0F && flag3 && (flag || flag2)) {
            Vector3d vector3d1 = collideBoundingBoxHeuristically(this,
                    new Vector3d(vec.x, this.stepHeight, vec.z), axisalignedbb, this.world, iselectioncontext,
                    reuseablestream, horizontal, vertical);
            Vector3d vector3d2 = collideBoundingBoxHeuristically(this,
                    new Vector3d(0.0D, this.stepHeight, 0.0D), axisalignedbb.expand(vec.x, 0.0D, vec.z),
    ...
    private boolean isInRain() {
        ...
        return this.world.isRainingAt(blockpos) || this.world.isRainingAt(new BlockPos(blockpos.getX(), this.getBoundingBox().maxY, blockpos.getZ()));
    }
    ...
    public void moveRelative(float friction, Vector3d relative) {
        StrafeEvent event = new StrafeEvent(friction, relative, this.rotationYaw);
        if (this instanceof ClientPlayerEntity) {
            event.hook();
        ...
        Vector3d vector3d = getAbsoluteMotion(event.getRelative(), event.getFriction(), event.getYaw());
    ...
    private static Vector3d getAbsoluteMotion(Vector3d relative, float p_213299_1_, float facing) {
        ...
        if (d0 < 1.0E-7D) {
            ...
            Vector3d vector3d = (d0 > 1.0D ? relative.normalize() : relative).scale(p_213299_1_);
    ...
    public float getDistanceVec(double x, double y, double z) {
    ...
    public final Vector3d getEyePosition(float partialTicks) {
        if (partialTicks == 1.0F) {
            ...
            double d0 = MathHelper.lerp(partialTicks, this.prevPosX, this.getPosX());
            double d1 = MathHelper.lerp(partialTicks, this.prevPosY, this.getPosY()) + (double) this.getEyeHeight();
            double d2 = MathHelper.lerp(partialTicks, this.prevPosZ, this.getPosZ());
            return new Vector3d(d0, d1, d2);
        }
    }

    public Vector3d getEyePos(float partialTicks) {
        return this.getEyePosition(partialTicks);
    }

    public final Vector3d getInterpolatedPosition(float partialTicks) {
        double d0 = MathHelper.lerp(partialTicks, this.prevPosX, this.getPosX());
        double d1 = MathHelper.lerp(partialTicks, this.prevPosY, this.getPosY());
        double d2 = MathHelper.lerp(partialTicks, this.prevPosZ, this.getPosZ());
        return new Vector3d(d0, d1, d2);
    }
    ...
    public boolean writeUnlessPassenger(CompoundNBT compound) {
        return !this.isPassenger() && this.writeUnlessRemoved(compound);
    }
    ...
    public float getCollisionBorderSize() {
        EntityHitBoxEvent event = new EntityHitBoxEvent(this, 0.0F);
        event.hook();
    ...
    public boolean isInvisibleToPlayer(PlayerEntity player) {
        if (SeeInvisibles.getInstance().isEnabled() && !(this instanceof ArmorStandEntity))
            return false;
    ...
    public Vector3d func_241205_ce_() {
        return new Vector3d(0.0D, this.getEyeHeight(), this.getWidth() * 0.4F);
    }
                ...
                for (int i2 = k; i2 < l; ++i2) {
                    for (int j2 = i1; j2 < j1; ++j2) {
                        ...
                        if (fluidstate.isTagged(fluidTag)) {
                            double d1 = (float) i2 + fluidstate.getActualHeight(this.world, blockpos$mutable);
    ...
    public org.joml.Vector3d getCustomPositionVector() {
        return new org.joml.Vector3d(positionVec.x, positionVec.y, positionVec.z);
    }
    ...
    public Vector3d getLeashPosition(float partialTicks) {
        return this.getInterpolatedPosition(partialTicks).add(0.0D, (double) this.eyeHeight * 0.7D, 0.0D);
    }
    ...
    public Vector3d getPositionVec(float partialTicks) {
        return new Vector3d(lastTickPosX, lastTickPosY, lastTickPosZ).lerp(getPositionVec(), partialTicks);
    }
}
