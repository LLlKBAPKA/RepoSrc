...
import org.excellent.client.managers.events.render.RenderWorldEvent;
import org.excellent.client.managers.events.world.WorldChangeEvent;
import org.excellent.client.managers.module.impl.render.CustomWorld;
...
public class GameRenderer implements IResourceManagerReloadListener, AutoCloseable {
    ...
    private void applyBobbing(MatrixStack matrixStackIn, float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof PlayerEntity playerentity) {
    ...
    public Matrix4f getProjectionMatrix(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFovSetting) {
        ...
        event.hook();
        matrixstack.getLast().getMatrix().mul(Matrix4f.perspective(this.getFOVModifier(activeRenderInfoIn, partialTicks, useFovSetting), event.getAspectRatio(), 0.05F, this.clipDistance));
                ...
                if (!this.mc.gameSettings.hideGUI || this.mc.currentScreen != null) {
                    ...
                    if (!firstGameFocused) {
                        if (!mc.gameSettings.showDebugInfo) {
                            ScaleMath.scalePre();
                            Instance.getComponent(DragComponent.class).post(matrixstack);
