...
import net.mojang.blaze3d.vertex.MatrixApplyingVertexBuilder;
...
public class ItemRenderer implements IResourceManagerReloadListener {
    ...
    public ItemRenderer(TextureManager textureManagerIn, ModelManager modelManagerIn, ItemColors itemColorsIn) {
        cppInit(textureManagerIn, modelManagerIn, itemColorsIn);
    }


    private void cppInit(TextureManager textureManagerIn, ModelManager modelManagerIn, ItemColors itemColorsIn) {
    ...
    public void renderModel(IBakedModel modelIn, ItemStack stack, int combinedLightIn, int combinedOverlayIn, MatrixStack matrixStackIn, IVertexBuilder bufferIn) {
        ...
        // Optimize by reducing repeated method calls and minimizing state changes
        for (Direction direction : Direction.VALUES) {
            random.setSeed(seed);
            List<BakedQuad> quads = modelIn.getQuads(null, direction, random);
            if (!quads.isEmpty()) {
                this.renderQuads(matrixStackIn, bufferIn, quads, stack, combinedLightIn, combinedOverlayIn);
            }
        }

        random.setSeed(seed);
        List<BakedQuad> generalQuads = modelIn.getQuads(null, null, random);
        if (!generalQuads.isEmpty()) {
            this.renderQuads(matrixStackIn, bufferIn, generalQuads, stack, combinedLightIn, combinedOverlayIn);
        }
    }
    ...
    public void renderItem(ItemStack itemStackIn, ItemCameraTransforms.TransformType transformTypeIn, boolean leftHand, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn, IBakedModel modelIn) {
        if (!itemStackIn.isEmpty()) {
            matrixStackIn.push();
            boolean isGuiTransform = transformTypeIn == ItemCameraTransforms.TransformType.GUI || transformTypeIn == ItemCameraTransforms.TransformType.GROUND || transformTypeIn == ItemCameraTransforms.TransformType.FIXED;

            if (itemStackIn.getItem() == Items.TRIDENT && isGuiTransform) {
            ...
            if (!modelIn.isBuiltInRenderer() && (itemStackIn.getItem() != Items.TRIDENT || isGuiTransform)) {
                boolean renderSolid = !(transformTypeIn != ItemCameraTransforms.TransformType.GUI && !transformTypeIn.isFirstPerson() && itemStackIn.getItem() instanceof BlockItem && (((BlockItem) itemStackIn.getItem()).getBlock() instanceof BreakableBlock || ((BlockItem) itemStackIn.getItem()).getBlock() instanceof StainedGlassPaneBlock));

                if (!modelIn.isLayered()) {
                    RenderType renderType = RenderTypeLookup.func_239219_a_(itemStackIn, renderSolid);
                    ...
                    if (itemStackIn.getItem() == Items.COMPASS && itemStackIn.hasEffect()) {
                        ...
                        if (transformTypeIn == ItemCameraTransforms.TransformType.GUI) {
                            matrixEntry.getMatrix().mul(0.5F);
                        } else if (transformTypeIn.isFirstPerson()) {
                            matrixEntry.getMatrix().mul(0.75F);
                        }

                        vertexBuilder = renderSolid ? getDirectGlintVertexBuilder(bufferIn, renderType, matrixEntry) : getGlintVertexBuilder(bufferIn, renderType, matrixEntry);
                        matrixStackIn.pop();
                    } else {
                        vertexBuilder = renderSolid ? getEntityGlintVertexBuilder(bufferIn, renderType, true, itemStackIn.hasEffect()) : getBuffer(bufferIn, renderType, true, itemStackIn.hasEffect());
                    }
                    ...
                    this.renderModel(modelIn, itemStackIn, combinedLightIn, combinedOverlayIn, matrixStackIn, vertexBuilder);

                    if (EmissiveTextures.isActive()) {
                        if (EmissiveTextures.hasEmissive()) {
                            EmissiveTextures.beginRenderEmissive();
                            IVertexBuilder emissiveVertexBuilder = vertexBuilder instanceof VertexBuilderWrapper ? ((VertexBuilderWrapper) vertexBuilder).getVertexBuilder() : vertexBuilder;
                            this.renderModel(modelIn, itemStackIn, LightTexture.MAX_BRIGHTNESS, combinedOverlayIn, matrixStackIn, emissiveVertexBuilder);
    ...
    public void renderItem(ItemStack itemStackIn, ItemCameraTransforms.TransformType transformTypeIn, int combinedLightIn, int combinedOverlayIn, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn) {
        this.renderItem(null, itemStackIn, transformTypeIn, false, matrixStackIn, bufferIn, null, combinedLightIn, combinedOverlayIn);
    }
    ...
    private boolean firstSetupTexture = true;
    ...
    public void renderItemAndEffectIntoGuiWithoutEntity(ItemStack stack, int x, int y) {
        this.renderItemIntoGUI(null, stack, x, y);
    }
    ...
    private void renderItemIntoGUI(@Nullable LivingEntity livingEntity, ItemStack stack, int x, int y) {
        if (!stack.isEmpty()) {
            ...
            try {
                this.renderItemModelIntoGUI(stack, x, y, this.getItemModelWithOverrides(stack, null, livingEntity));
