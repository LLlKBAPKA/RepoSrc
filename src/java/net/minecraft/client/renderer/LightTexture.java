    ...
    public void updateLightmap(float partialTicks) {
        if (this.needsUpdate) {
            ...
            if (clientworld != null) {
                ...
                if (this.client.player.isPotionActive(Effects.NIGHT_VISION) || (FullBright.getInstance().isEnabled() && FullBright.getInstance().mode().is("Ночное Зрение"))) {
                ...
                for (int i = 0; i < 16; ++i) {
                    for (int j = 0; j < 16; ++j) {
                        ...
                        float f11 = FullBright.getInstance().isEnabled() && FullBright.getInstance().mode().is("Гамма") ? 10 : (float) this.client.gameSettings.gamma;
