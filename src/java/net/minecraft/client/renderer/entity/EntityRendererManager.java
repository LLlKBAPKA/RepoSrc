...
public class EntityRendererManager {
    ...
    public EntityRendererManager(TextureManager textureManagerIn, net.minecraft.client.renderer.ItemRenderer itemRendererIn, IReloadableResourceManager resourceManagerIn, FontRenderer fontRendererIn, GameSettings gameSettingsIn) {
        cppInit(textureManagerIn, itemRendererIn, resourceManagerIn, fontRendererIn, gameSettingsIn);
    }


    private void cppInit(TextureManager textureManagerIn, net.minecraft.client.renderer.ItemRenderer itemRendererIn, IReloadableResourceManager resourceManagerIn, FontRenderer fontRendererIn, GameSettings gameSettingsIn) {
    ...
    public <E extends Entity> void renderClearEntityStatic(E entityIn, double xIn, double yIn, double zIn, float rotationYawIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        if (this.info != null) {
            EntityRenderer<? super E> entityrenderer = this.getRenderer(entityIn);
            ...
            try {
                ...
                new EntityRenderMatrixEvent(matrixStackIn, entityIn).hook();
    ...
    private void renderDebugBoundingBox(MatrixStack matrixStackIn, IVertexBuilder bufferIn, Entity entityIn, float partialTicks) {
        if (!Shaders.isShadowPass) {
            ...
            if (flag) {
                ...
                Entity[] aentity = Reflector.IForgeEntity_getParts.exists() ? (Entity[]) Reflector.call(entityIn, Reflector.IForgeEntity_getParts) : ((EnderDragonEntity) entityIn).getDragonParts();

                for (Entity entity : aentity) {
                    matrixStackIn.push();
                    double d3 = d0 + MathHelper.lerp(partialTicks, entity.lastTickPosX, entity.getPosX());
                    double d4 = d1 + MathHelper.lerp(partialTicks, entity.lastTickPosY, entity.getPosY());
                    double d5 = d2 + MathHelper.lerp(partialTicks, entity.lastTickPosZ, entity.getPosZ());
    ...
    private static void renderShadow(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, Entity entityIn, float weightIn, float partialTicks, IWorldReader worldIn, float sizeIn) {
        if (!Config.isShaders() || !Shaders.shouldSkipDefaultShadow) {
            ...
            if (entityIn instanceof MobEntity mobentity) {
            ...
            double d2 = MathHelper.lerp(partialTicks, entityIn.lastTickPosX, entityIn.getPosX());
            double d0 = MathHelper.lerp(partialTicks, entityIn.lastTickPosY, entityIn.getPosY());
            double d1 = MathHelper.lerp(partialTicks, entityIn.lastTickPosZ, entityIn.getPosZ());
