...
import org.excellent.client.managers.module.impl.render.HitColor;
...
public abstract class LivingRenderer<T extends LivingEntity, M extends EntityModel<T>> extends EntityRenderer<T> implements IEntityRenderer<T, M> {
    ...
    public void render(T entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        ...
        if (this.entityModel.isSitting && entityIn.getRidingEntity() instanceof LivingEntity livingentity) {
        ...
        if (rendertype != null) {
            ...
            SeeInvisibles module = SeeInvisibles.getInstance();

            float red = 1F;
            float green = 1F;
            float blue = 1F;
            if (entityIn instanceof PlayerEntity && HitColor.getInstance().isEnabled() && entityIn.hurtTime > 0) {
                float hurtPC = (float) Math.sin(entityIn.hurtTime * (18F * Math.PI / 180F));
                float[] rgb = ColorUtil.getRGBf(ColorUtil.overCol(ColorUtil.getColor(red, green, blue), HitColor.getInstance().color().getValue(), hurtPC));
                red = rgb[0];
                green = rgb[1];
                blue = rgb[2];
            }

            this.entityModel.render(matrixStackIn, ivertexbuilder, packedLightIn, i, red, green, blue, flag2 ? (module.isEnabled() ? module.alpha().getValue() : 0.15F) : 1.0F);
        }

        if (!entityIn.isSpectator() && renderLayers) {
        ...
        RenderHeadLayerEvent event = RenderHeadLayerEvent.getInstance();
        event.set(entityIn, matrixStackIn, getEntityModel());
        event.hook();
    ...
    protected void applyRotations(T entityLiving, MatrixStack matrixStackIn, float ageInTicks, float rotationYaw, float partialTicks) {
        ...
        if (entityLiving.deathTime > 0) {
            ...
            if (entityLiving instanceof ClientPlayerEntity) {
                matrixStackIn.rotate(Vector3f.XP.rotationDegrees(-90.0F - entityLiving.rotationPitchHead));
            } else {
            ...
            String s = TextFormatting.removeFormatting(entityLiving.getName().getString());
