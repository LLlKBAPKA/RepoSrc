...
public class ChunkRenderDispatcher {
    ...
    public boolean updateTransparencyLater(ChunkRenderDispatcher.ChunkRender p_updateTransparencyLater_1_) {
        return !this.freeBuilders.isEmpty() && p_updateTransparencyLater_1_.resortTransparency(RenderTypes.TRANSLUCENT, this);
    }
    ...
    public class ChunkRender implements IForgeRenderChunk {
        ...
        private final boolean renderRegions = Config.isRenderRegions();
        public int regionX;
        public int regionZ;
        ...
        private Chunk chunk;
        private final ChunkRenderDispatcher.ChunkRender[] renderChunkNeighbours = new ChunkRenderDispatcher.ChunkRender[Direction.VALUES.length];
        private final ChunkRenderDispatcher.ChunkRender[] renderChunkNeighboursValid = new ChunkRenderDispatcher.ChunkRender[Direction.VALUES.length];
        private boolean renderChunkNeighboursUpated = false;
        private final WorldRenderer.LocalRenderInformationContainer renderInfo = new WorldRenderer.LocalRenderInformationContainer(this, null, 0);
        ...
        public void setPosition(int x, int y, int z) {
            if (x != this.position.getX() || y != this.position.getY() || z != this.position.getZ()) {
                ChunkPositionEvent event = new ChunkPositionEvent(x, y, z, this);
                event.hook();
                ...
                for (ChunkRender chunkrenderdispatcher$chunkrender : this.renderChunkNeighbours) {
        ...
        private boolean isWorldPlayerUpdate() {
            if (ChunkRenderDispatcher.this.world instanceof ClientWorld clientworld) {
            ...
            private Set<TileEntity> compile(float xIn, float yIn, float zIn, ChunkRenderDispatcher.CompiledChunk compiledChunkIn, RegionRenderCacheBuilder builderIn) {
                ...
                if (!ChunkRender.this.isChunkRegionEmpty(blockpos)) {
                    ...
                    for (RenderType rendertype2 : ChunkRenderDispatcher.BLOCK_RENDER_LAYERS) {
                        compiledChunkIn.setAnimatedSprites(rendertype2, null);
                    }
