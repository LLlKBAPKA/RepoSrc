...
import org.excellent.client.managers.events.player.ActionEvent;
import org.excellent.client.managers.events.player.MotionEvent;
import org.excellent.client.managers.events.player.PostUpdateEvent;
import org.excellent.client.managers.events.player.UpdateEvent;
import org.excellent.client.managers.module.impl.movement.AntiHunger;
import org.excellent.client.managers.module.impl.movement.Sprint;
import org.excellent.client.managers.module.impl.player.NoPush;
...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
    ...
    public int sneakTicks, standTicks;
    ...
    public void tick() {
        if (this.world.isBlockLoaded(new BlockPos(this.getPosX(), 0.0D, this.getPosZ()))) {
            UpdateEvent.getInstance().hook();
            super.tick();
            PostUpdateEvent.getInstance().hook();
            if (this.isPassenger()) {
    ...
    private void onUpdateWalkingPlayer() {
        ...
        MotionEvent motionEvent = new MotionEvent(posX, posY, posZ, yaw, pitch, onGround, isSneaking, isSprinting);
        ...
        yaw = motionEvent.getYaw();
        pitch = motionEvent.getPitch();

        onGround = motionEvent.isOnGround();
        isSneaking = motionEvent.isSneaking();
        isSprinting = motionEvent.isSprinting();

        boolean flag = isSprinting;
        ActionEvent actionEvent = new ActionEvent(flag);
        actionEvent.hook();
        flag = actionEvent.isSprintState();
        ...
        if (this.isCurrentViewEntity()) {
            double d4 = posX - this.lastReportedPosX;
            double d0 = posY - this.lastReportedPosY;
            double d1 = posZ - this.lastReportedPosZ;
            double d2 = yaw - this.lastReportedYaw;
            double d3 = pitch - this.lastReportedPitch;
            ...
            if (this.isPassenger()) {
                ...
                this.connection.sendPacket(new CPlayerPacket.PositionRotationPacket(vector3d.x, -999.0D, vector3d.z, yaw, pitch, onGround));
                flag1 = false;
            } else if (flag1 && flag2) {
                this.connection.sendPacket(new CPlayerPacket.PositionRotationPacket(posX, posY, posZ, yaw, pitch, onGround));
            } else if (flag1) {
                this.connection.sendPacket(new CPlayerPacket.PositionPacket(posX, posY, posZ, onGround));
            } else if (flag2) {
    ...
    public void sendChatMessage(String message) {
        ...
        this.connection.sendPacket(new CChatMessagePacket(event.getMessage()));
    }
    ...
    public void closeScreen() {
        ScreenCloseEvent event = new ScreenCloseEvent(mc.currentScreen, this.openContainer.windowId);
        event.hook();
    ...
    private boolean shouldBlockPushPlayer(BlockPos pos) {
        NoPush noPush = NoPush.getInstance();
        if (noPush.isEnabled() && noPush.checks().getValue("Блоков")) {
            return false;
        }
    ...
    public void openSignEditor(SignTileEntity signTile) {
        this.mc.displayScreen(new EditSignScreen(signTile));
    }

    public void openMinecartCommandBlock(CommandBlockLogic commandBlock) {
        this.mc.displayScreen(new EditMinecartCommandBlockScreen(commandBlock));
    }

    public void openCommandBlock(CommandBlockTileEntity commandBlock) {
        this.mc.displayScreen(new CommandBlockScreen(commandBlock));
    }

    public void openStructureBlock(StructureBlockTileEntity structure) {
        this.mc.displayScreen(new EditStructureScreen(structure));
    }

    public void openJigsaw(JigsawTileEntity p_213826_1_) {
        this.mc.displayScreen(new JigsawScreen(p_213826_1_));
    }
    ...
    public void openBook(ItemStack stack, Hand hand) {
        ...
        if (item == Items.WRITABLE_BOOK) {
            this.mc.displayScreen(new EditBookScreen(this, stack, hand));
        }
    }
    ...
    public void updateEntityActionState() {
        ...
        if (this.isCurrentViewEntity()) {
            ...
            if (FreeLookComponent.isActive()) {
                this.renderArmPitch = (float) ((double) this.renderArmPitch + (double) (mc.gameRenderer.getActiveRenderInfo().getPitch() - this.renderArmPitch) * 0.5D);
                this.renderArmYaw = (float) ((double) this.renderArmYaw + (double) (mc.gameRenderer.getActiveRenderInfo().getYaw() - this.renderArmYaw) * 0.5D);
            } else {
                this.renderArmPitch = (float) ((double) this.renderArmPitch + (double) (rotationPitch - this.renderArmPitch) * 0.5D);
                this.renderArmYaw = (float) ((double) this.renderArmYaw + (double) (rotationYaw - this.renderArmYaw) * 0.5D);
            }
        }
    }
    ...
    public void livingTick() {
        ...
        boolean flag4 = (float) this.getFoodStats().getFoodLevel() > 6.0F || abilities.allowFlying || AntiHunger.getInstance().isEnabled();
        ...
        if (Sprint.getInstance().isEnabled() && !this.isSprinting() && (!this.isInWater() || this.canSwim()) && this.isUsingSwimmingAnimation() && flag4 && (!this.isPotionActive(Effects.BLINDNESS))) {
    ...
    /**
     * Removes the given potion effect from the active potion map and returns it. Does not hook cleanup callbacks for
