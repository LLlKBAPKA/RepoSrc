...
import org.excellent.client.managers.events.other.EntitySpawnEvent;
...
public class ClientWorld extends World {
    ...
    private final ClientWorld.ClientWorldInfo clientWorldInfo;
    private final DimensionRenderInfo dimensionRenderInfo;
    ...
    public ClientWorld(ClientPlayNetHandler clientPlayNetHandler, ClientWorld.ClientWorldInfo clientWorldInfo, RegistryKey<World> worldRegistryKey, DimensionType dimensionType, int viewDistance, WorldRenderer worldRenderer, boolean isDebug, long seed) {
        super(clientWorldInfo, worldRegistryKey, dimensionType, true, isDebug, seed);
        this.connection = clientPlayNetHandler;
        this.clientChunkProvider = new ClientChunkProvider(this, viewDistance);
        this.clientWorldInfo = clientWorldInfo;
        this.worldRenderer = worldRenderer;
        this.dimensionRenderInfo = DimensionRenderInfo.func_243495_a(dimensionType);
        this.setSpawn(new BlockPos(8, 64, 8), 0.0F);
    ...
    public DimensionRenderInfo func_239132_a_() {
        return this.dimensionRenderInfo;
    }
    ...
    public void tick(BooleanSupplier hasTimeLeft) {
        ...
        this.clientChunkProvider.tick(hasTimeLeft);
    ...
    public void func_239134_a_(long p_239134_1_) {
        this.clientWorldInfo.setGameTime(p_239134_1_);
    }
    ...
    public void setDayTime(long time) {
        ...
        this.clientWorldInfo.setDayTime(time);
    }
    ...
    public List<LivingEntity> loadedLivingEntityList() {
    ...
    public List<Entity> loadedItemEntityList() {
    ...
    public void onChunkUnloaded(Chunk chunkIn) {
        ...
        this.clientChunkProvider.getLightManager().enableLightSources(chunkIn.getPos(), false);
    }
    ...
    private void addEntityImpl(int entityIdIn, Entity entityToSpawn) {
        if (!Reflector.EntityJoinWorldEvent_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.EntityJoinWorldEvent_Constructor, entityToSpawn, this)) {
            ...
            new EntitySpawnEvent(entityToSpawn).hook();
        }
    }
    ...
    public void removeEntityFromWorld(int eid) {
        ...
        if (entity != null) {
            new EntityRemoveEvent(entity).hook();
            entity.remove();
            this.removeEntity(entity);
        }
    }
    ...
    private void spawnFluidParticle(BlockPos blockPosIn, BlockState blockStateIn, IParticleData particleDataIn, boolean shapeDownSolid) {
        if (blockStateIn.getFluidState().isEmpty()) {
            ...
            if (d0 < 1.0D) {
                if (shapeDownSolid) {
                    this.spawnParticle(blockPosIn.getX(), blockPosIn.getX() + 1, blockPosIn.getZ(), blockPosIn.getZ() + 1, (double) (blockPosIn.getY() + 1) - 0.05D, particleDataIn);
                }
    ...
    public Vector3d getSkyColor(BlockPos blockPosIn, float partialTicks) {
        ...
        WorldColorEvent worldColorEvent = new WorldColorEvent(f2, f3, f4);
        worldColorEvent.hook();
        float eventRed = worldColorEvent.getRed();
        float eventGreen = worldColorEvent.getGreen();
        float eventBlue = worldColorEvent.getBlue();

        return new Vector3d(eventRed, eventGreen, eventBlue);
    }
    ...
    public void setSpawn(BlockPos p_239136_1_, float p_239136_2_) {
