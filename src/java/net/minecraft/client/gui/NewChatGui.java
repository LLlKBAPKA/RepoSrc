...
import org.excellent.client.managers.module.impl.misc.BetterMinecraft;
...
public class NewChatGui extends AbstractGui {
    ...
    private ChatLine<ITextComponent> lastChatLine;
        ...
        if (!this.func_238496_i_()) {
            ...
            if (k > 0) {
                ...
                for (int j1 = 0; j1 + this.scrollPos < this.drawnChatLines.size() && j1 < j; ++j1) {
                    ...
                    if (chatline != null) {
                        int k1 = p_238492_2_ - chatline.getUpdateCounterCreated();
                        ...
                        if (k1 < 200 || flag) {
                            ...
                            if (i2 > 3) {
                                ...
                                if (this.mc.gameSettings.ofChatBackground != 3) {
                                    fill(p_238492_1_, -2, (int) (d6 - d3), l + 4, (int) d6, j2 << 24);
                                }
                                ...
                                if (!this.mc.gameSettings.ofChatShadow) {
                                    this.mc.fontRenderer.drawString(p_238492_1_, chatline.getLineString(), 0.0F, (float) ((int) (d6 + d4)), 16777215 + (i2 << 24));
                                } else {
                                    this.mc.fontRenderer.drawStringWithShadow(p_238492_1_, chatline.getLineString(), 0.0F, (float) ((int) (d6 + d4)), 16777215 + (i2 << 24));
                                }
                ...
                if (!this.field_238489_i_.isEmpty()) {
                    ...
                    RenderSystem.enableBlend();
                    p_238492_1_.translate(0.0D, 0.0D, 50.0D);
                    this.mc.fontRenderer.drawStringWithShadow(p_238492_1_, new TranslationTextComponent("chat.queue", this.field_238489_i_.size()), 0.0F, 1.0F, 16777215 + (l2 << 24));
                    p_238492_1_.pop();
                    RenderSystem.disableAlphaTest();
                    RenderSystem.disableBlend();
                }
    ...
    public void printChatMessage(ITextComponent chatComponent) {
        if (BetterMinecraft.getInstance().isEnabled() && BetterMinecraft.getInstance().antiSpam().getValue()) {
            String newMessage = chatComponent.getString();
            if (lastChatLine != null && lastChatLine.getLineString().getString().contains(newMessage)) {
                repeatCount++;
                ITextComponent updatedMessage = chatComponent.deepCopy().appendString(TextFormatting.GRAY + " (x" + repeatCount + ")");
                deleteChatLine(lastChatLine.getChatLineID());
                this.func_238493_a_(updatedMessage, lastChatLine.getChatLineID(), this.mc.ingameGUI.getTicks(), false);
                lastChatLine = new ChatLine<>(this.mc.ingameGUI.getTicks(), updatedMessage, lastChatLine.getChatLineID(), false);
                refreshChat();
            } else {
                repeatCount = 1;
                int currentChatLineID = nextChatLineID++;
                this.func_238493_a_(chatComponent, currentChatLineID, this.mc.ingameGUI.getTicks(), false);
                lastChatLine = new ChatLine<>(this.mc.ingameGUI.getTicks(), chatComponent, currentChatLineID, false);
            }
    ...
    private void func_238493_a_(ITextComponent component, int p_238493_2_, int p_238493_3_, boolean p_238493_4_) {
        // Метод для добавления сообщений в чат
        int i = MathHelper.floor((double) this.getChatWidth() / this.getScale());
        List<IReorderingProcessor> list = RenderComponentsUtil.func_238505_a_(component, i, this.mc.fontRenderer);
        boolean flag = this.getChatOpen();
        ...
        if (!p_238493_4_) {
            this.chatLines.add(0, new ChatLine<>(p_238493_3_, component, p_238493_2_, false));
    ...
    public void refreshChat() {
        ...
        for (int i = this.chatLines.size() - 1; i >= 0; --i) {
            ...
            this.func_238493_a_(chatline.getLineString(), chatline.getChatLineID(), chatline.getUpdateCounterCreated(), true);
        }
    }
    ...
    public int getChatWidth() {
        ...
        int j = (int) ((double) (mainwindow.getFramebufferWidth() - 3) / mainwindow.getScaleFactor());
