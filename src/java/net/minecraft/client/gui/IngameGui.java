...
import org.excellent.client.managers.events.render.RenderCrosshairEvent;
import org.excellent.client.managers.events.render.RenderScoreBoardEvent;
import org.excellent.client.managers.module.impl.player.FreeCam;
...
public class IngameGui extends AbstractGui {
    ...
    private DebugOverlayGui overlayDebug;
    private SubtitleOverlayGui overlaySubtitle;

    /**
     * The spectator GUI for this in-game GUI instance
     */
    private SpectatorGui spectatorGui;
    public PlayerTabOverlayGui overlayPlayerList;
    private BossOverlayGui overlayBoss;
    ...
    private void cppInit(Minecraft mcIn) {
        this.mc = mcIn;
    ...
    public void renderIngameGui(MatrixStack matrixStack, float partialTicks) {
        ...
        if (this.mc.gameSettings.getPointOfView().firstPerson() && itemstack.getItem() == Blocks.CARVED_PUMPKIN.asItem()) {
        ...
        if (!this.mc.gameSettings.hideGUI) {
            ...
            if (this.mc.gameSettings.heldItemTooltips && (FreeCam.getInstance().isEnabled() || this.mc.playerController.getCurrentGameType() != GameType.SPECTATOR)) {
...
//        this.renderPotionIcons(matrixStack);
        ...
        if (!this.mc.gameSettings.hideGUI) {
            if (this.overlayMessage != null && this.overlayMessageTime > 0) {
                ...
                if (i1 > 8) {
                    ...
                    fontrenderer.drawString(matrixStack, this.overlayMessage, (float) (-l / 2), -4.0F, k1 | k);
                    RenderSystem.disableBlend();
            ...
            if (this.displayedTitle != null && this.titlesTimer > 0) {
                ...
                if (j1 > 8) {
                    ...
                    fontrenderer.drawStringWithShadow(matrixStack, this.displayedTitle, (float) (-i2 / 2), -10.0F, 16777215 | l1);
                    RenderSystem.popMatrix();
                    ...
                    if (this.displayedSubTitle != null) {
                        ...
                        fontrenderer.drawStringWithShadow(matrixStack, this.displayedSubTitle, (float) (-k2 / 2), 5.0F, 16777215 | l1);
                        RenderSystem.popMatrix();
                    }
            ...
            if (scoreobjective1 != null) {
                Scoreboard sbb = scoreobjective1.getScoreboard();
                Collection<Score> collection = sbb.getSortedScores(scoreobjective1);
                ...
                for (Score score : collection) {
                    ScorePlayerTeam spt = sbb.getPlayersTeam(score.getPlayerName());
                    ITextComponent itextcomponent1 = ScorePlayerTeam.func_237500_a_(spt, new StringTextComponent(score.getPlayerName()));
                    list1.add(Pair.of(score, itextcomponent1));
                }

                ScoreBoardEvent event = new ScoreBoardEvent(list1);
                event.hook();
            }

            if (scoreobjective1 != null && (!NoRender.getInstance().isEnabled() || !NoRender.getInstance().elements().getValue("Скорборд"))) {
                RenderScoreBoardEvent event = new RenderScoreBoardEvent(matrixStack, scoreobjective1, getFontRenderer(), scaledWidth, scaledHeight);
    ...
    private void func_238456_d_(MatrixStack matrixStack) {
        ...
        if (gamesettings.getPointOfView().firstPerson() && (this.mc.playerController.getCurrentGameType() != GameType.SPECTATOR || this.isTargetNamedMenuProvider(this.mc.objectMouseOver))) {
            if (gamesettings.showDebugInfo && !gamesettings.hideGUI && !this.mc.player.hasReducedDebug() && !gamesettings.reducedDebugInfo) {
                RenderSystem.pushMatrix();
                RenderSystem.translatef(this.scaledWidth / 2F, this.scaledHeight / 2F, (float) this.getBlitOffset());
                ...
                RenderCrosshairEvent renderCrosshairEvent = new RenderCrosshairEvent(matrixStack, mc.getRenderPartialTicks(), this.scaledWidth / 2F, this.scaledHeight / 2F);
                renderCrosshairEvent.hook();
                if (renderCrosshairEvent.isCancelled()) return;
                RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.ONE_MINUS_DST_COLOR, GlStateManager.DestFactor.ONE_MINUS_SRC_COLOR, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
                float i = 16;
                RenderSystem.translatef(((this.scaledWidth - i) / 2F), ((this.scaledHeight - i) / 2F), 0.0F);
                this.blit(matrixStack, 0.5F, 0.5F, 0, 0, i, i);
                RenderSystem.translatef(-((this.scaledWidth - i) / 2F), -((this.scaledHeight - i) / 2F), 0.0F);
    ...
    public void func_238453_b_(MatrixStack p_238453_1_) {
        ...
        if (this.remainingHighlightTicks > 0 && !this.highlightingItemStack.isEmpty()) {
            ...
            if (l > 0) {
                ...
                this.getFontRenderer().drawStringWithShadow(p_238453_1_, itextcomponent, (float) j, (float) k, 16777215 + (l << 24));
    ...
    public void func_238455_c_(MatrixStack p_238455_1_) {
        ...
        int i = this.getFontRenderer().getStringPropertyWidth(itextcomponent);
        this.getFontRenderer().drawStringWithShadow(p_238455_1_, itextcomponent, (float) (this.scaledWidth - i - 10), 5.0F, 16777215);
    ...
    private void func_238447_a_(MatrixStack p_238447_1_, ScoreObjective p_238447_2_) {
        ...
        for (Pair<Score, ITextComponent> pair : list1) {
            ...
            this.getFontRenderer().drawString(p_238447_1_, itextcomponent2, (float) l2, (float) k1, -1);

//            red numbers
//            this.getFontRenderer().drawStringWithShadow(p_238447_1_, s, (float) (l1 - this.getFontRenderer().getStringWidth(s)), (float) k1, -1);

            if (l == collection.size()) {

                fill(p_238447_1_, l2 - 2, k1 - 9 - 1, l1, k1 - 1, j1);
                fill(p_238447_1_, l2 - 2, k1 - 1, l1, k1, i1);
                this.getFontRenderer().drawString(p_238447_1_, itextcomponent, (float) (l2 + j / 2 - i / 2), (float) (k1 - 9), -1);
            }
        }
    }
    ...
    private void renderPumpkinOverlay() {
        ...
        RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
        RenderSystem.disableAlphaTest();
        drawTexture(PUMPKIN_BLUR_TEX_PATH);
        RenderSystem.enableAlphaTest();
        RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
    }
    ...
    private void renderVignette(Entity entityIn) {
        if (!Config.isVignetteEnabled()) {
            ...
            drawTexture(VIGNETTE_TEX_PATH);
            RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
            RenderSystem.defaultBlendFunc();
        }
    }

    private void drawTexture(ResourceLocation vignetteTexPath) {
        this.mc.getTextureManager().bindTexture(vignetteTexPath);
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
        bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX);
        bufferbuilder.pos(0.0D, this.scaledHeight, -90.0D).tex(0.0F, 1.0F).endVertex();
        bufferbuilder.pos(this.scaledWidth, this.scaledHeight, -90.0D).tex(1.0F, 1.0F).endVertex();
        bufferbuilder.pos(this.scaledWidth, 0.0D, -90.0D).tex(1.0F, 0.0F).endVertex();
    ...
    public void handleChatPacket(ChatType chatType, ITextComponent textComponent, UUID uuid) {
        if (this.mc.cannotSendChatMessages(uuid)) {
            for (IChatListener ichatlistener : this.chatListeners.get(chatType)) {
                ichatlistener.say(chatType, textComponent, uuid);
            }
        }
    }
