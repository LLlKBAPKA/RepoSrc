...
public class DebugOverlayGui extends AbstractGui {
    ...
    public void render(MatrixStack matrixStack) {
        ...
        if (this.mc.gameSettings.showLagometer) {
            ...
            if (integratedserver != null) {
                this.func_238509_a_(matrixStack, integratedserver.getFrameTimer(), i - Math.min(i / 2, 240), i / 2, false);
            }
        }
    }
    ...
    protected void renderDebugInfoLeft(MatrixStack matrixStack) {
        ...
        if (list == null || System.currentTimeMillis() > this.updateInfoLeftTimeMs) {
            ...
            list.add("Debug: Pie [shift]: " + (flag ? "FPS + TPS" : " FPS") + " [alt]: " + (this.mc.gameSettings.showLagometer ? "visible" : "hidden"));
            list.add("For help: press F3 + Q");
            list.add(String.format("[%s-via] -> %s (%s)", Constants.NAMESPACE, ViaLoadingBase.getInstance().getTargetVersion().getName(), ViaLoadingBase.getInstance().getTargetVersion().getVersion()));
        ...
        float offset = 0;
        for (String l : list) {
            fontRenderer.drawStringWithShadow(matrixStack, l, 5, 5 + offset, -1);
    ...
    protected void renderDebugInfoRight(MatrixStack matrixStack) {
        ...
        float offset = 0;
        for (String l : list) {
            fontRenderer.drawStringWithShadow(matrixStack, l, this.mc.getMainWindow().getScaledWidth() - 5 - fontRenderer.getStringWidth(l), 5 + offset, -1);
            offset += fontRenderer.FONT_HEIGHT;
        }
    }
        ...
        if (this.mc.isReducedDebug()) {
            ...
            if (serverworld != null) {
                ...
                if (worldentityspawner$entitydensitymanager != null) {
                    ...
                    list.add("SC: " + l + ", " + Stream.of(EntityClassification.values()).map((p_lambda$getInfoLeft$1_1_) ->
                    {
    ...
    protected List<String> getDebugInfoRight() {
        ...
        if (Reflector.BrandingControl_getBrandings.exists()) {
            ...
            for (String s1 : (Set<String>) Reflector.call(Reflector.BrandingControl_getBrandings, true, false)) {
        ...
        if (this.mc.isReducedDebug()) {
            ...
            if (this.rayTraceFluid.getType() == RayTraceResult.Type.BLOCK) {
                ...
                list.add(String.valueOf(Registry.FLUID.getKey(fluidstate.getFluid())));
    ...
    private void func_238509_a_(MatrixStack p_238509_1_, FrameTimer p_238509_2_, int p_238509_3_, int p_238509_4_, boolean p_238509_5_) {
        if (!p_238509_5_) {
            int i = (int) (512.0D / this.mc.getMainWindow().getScaleFactor());
