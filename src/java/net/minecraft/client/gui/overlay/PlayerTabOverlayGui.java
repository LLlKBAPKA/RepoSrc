...
public class PlayerTabOverlayGui extends AbstractGui {
    public static final Ordering<NetworkPlayerInfo> ENTRY_ORDERING = Ordering.from(new PlayerTabOverlayGui.PlayerComparator());
    ...
    public void func_238523_a_(MatrixStack p_238523_1_, int p_238523_2_, Scoreboard p_238523_3_, @Nullable ScoreObjective p_238523_4_) {
        ...
        for (ScorePlayerTeam team : mc.world.getScoreboard().getTeams().stream().sorted(Comparator.comparing(Team::getName)).toList()) {
            ...
            if (PlayerUtil.NAME_REGEX.matcher(name).matches() && !team.getPrefix().getString().isEmpty()) {
                boolean vanish = true;
                ...
                if (vanish) {
                    StringTextComponent vanishedText = new StringTextComponent(TextFormatting.GRAY + "[" + TextFormatting.RED + "V" + TextFormatting.GRAY + "] ");
        ...
        if (list1 != null) {
            ...
            for (IReorderingProcessor ireorderingprocessor2 : list1) {
                ...
                this.mc.fontRenderer.drawStringWithShadow(p_238523_1_, ireorderingprocessor2, (float) (p_238523_2_ / 2 - i2 / 2), (float) k1, -1);
                k1 += 9;
            }
        ...
        for (int i5 = 0; i5 < i4; ++i5) {
            ...
            if (i5 < list.size()) {
                ...
                if (Globals.getInstance().isEnabled()) {
                    String displayName = this.getDisplayName(networkplayerinfo1).getString();
                    for (String user : ClientAPI.USERS.keySet()) {
                        if (displayName.contains(user)) {
                            String userImageURL = "https://rockstar.moscow/api/globals/" + ClientAPI.USERS.get(user) + ".png";
                            mc.getTextureManager().bindTexture(Converter.getResourceLocation(userImageURL));
                            RectUtil.drawRect(p_238523_1_, k2, l2, 8, 8, -1, false, true);
                            clientUser = true;
                            break;
                        }
                    }
                ...
                this.mc.fontRenderer.drawStringWithShadow(p_238523_1_, this.getDisplayName(networkplayerinfo1), (float) k2 + (clientUser ? 12 : 0), (float) l2, networkplayerinfo1.getGameType() == GameType.SPECTATOR ? -1862270977 : -1);
        ...
        if (list2 != null) {
            ...
            for (IReorderingProcessor ireorderingprocessor3 : list2) {
                ...
                this.mc.fontRenderer.drawStringWithShadow(p_238523_1_, ireorderingprocessor3, (float) (p_238523_2_ / 2 - k5 / 2), (float) k1, -1);
                k1 += 9;
            }
        }
    }
        ...
        if (objective.getRenderType() == ScoreCriteria.RenderType.HEARTS) {
            ...
            if (this.lastTimeOpened == info.getRenderVisibilityId()) {
                if (i < info.getLastHealth()) {
                    ...
                    info.setHealthBlinkTime(this.guiIngame.getTicks() + 20);
                } else if (i > info.getLastHealth()) {
                    info.setLastHealthTime(j);
                    info.setHealthBlinkTime(this.guiIngame.getTicks() + 10);
                }
            }
