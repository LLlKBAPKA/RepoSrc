...
public class DeathScreen extends Screen {
    ...
    protected void init() {
        ...
        Button button = this.addButton(new Button(this.width / 2 - 100, this.height / 4 + 96, 200, 20, new TranslationTextComponent("deathScreen.titleScreen"), (p_213020_1_) ->
        {
            if (this.isHardcoreMode) {
                ...
                this.minecraft.displayScreen(confirmscreen);
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredStringWithShadow(matrixStack, this.font, this.title, this.width / 2 / 2, 30, 16777215);
        RenderSystem.popMatrix();

        if (this.causeOfDeath != null) {
            drawCenteredStringWithShadow(matrixStack, this.font, this.causeOfDeath, this.width / 2, 85, 16777215);
        }

        drawCenteredStringWithShadow(matrixStack, this.font, this.field_243285_p, this.width / 2, 100, 16777215);
