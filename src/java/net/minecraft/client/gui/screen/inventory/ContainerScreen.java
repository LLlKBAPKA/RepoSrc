...
import org.excellent.client.managers.module.impl.misc.ItemScroller;
...
public abstract class ContainerScreen<T extends Container> extends Screen implements IHasContainer<T> {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        new ContainerRenderEvent(matrixStack, this.title, this.container).hook();
        ...
        for (int i1 = 0; i1 < this.container.inventorySlots.size(); ++i1) {
            ...
            if (this.isSlotSelected(slot, mouseX, mouseY) && slot.isEnabled()) {
                ...
                final MainWindow mw = minecraft.getMainWindow();
                if (scroller.isEnabled()) {
                    if (minecraft.currentScreen != null
                            && GLFW.glfwGetMouseButton(mw.getHandle(), GLFW.GLFW_MOUSE_BUTTON_LEFT) == GLFW.GLFW_PRESS
                            && GLFW.glfwGetKey(mw.getHandle(), GLFW.GLFW_KEY_LEFT_SHIFT) == GLFW.GLFW_PRESS) {
                        if (scroller.time().finished(scroller.delay().getValue().doubleValue()) && slot.getHasStack()) {
    ...
    protected void drawGuiContainerForegroundLayer(MatrixStack matrixStack, int x, int y) {
        ...
        this.font.drawString(matrixStack, this.playerInventory.getDisplayName(), (float) this.playerInventoryTitleX, (float) this.playerInventoryTitleY, 4210752);
    }
        ...
        if (super.keyPressed(keyCode, scanCode, modifiers)) {
            ...
            if (this.hoveredSlot != null && this.hoveredSlot.getHasStack()) {
                if (this.minecraft.gameSettings.keyBindPickBlock.matchesKey(keyCode, scanCode)) {
                    ...
                    if (ItemScroller.getInstance().isEnabled() && hasShiftDown() && hasControlDown()) {
                        ItemStack stack = this.hoveredSlot.getStack();
                        List<Integer> slots = Lists.newArrayList();

                        for (int i = 0; i < this.container.getInventory().size(); i++) {
                            ItemStack slot = this.container.getInventory().get(i);
                            if (slot.getItem().equals(stack.getItem())) {
                                slots.add(i);
                            }
                        }

                        slots.forEach(slot -> this.minecraft.playerController.windowClick(this.container.windowId, slot, 1, ClickType.THROW, this.minecraft.player));
                        return true;
                    }
