...
public class InventoryScreen extends DisplayEffectsScreen<PlayerContainer> implements IRecipeShownListener {
    ...
    public void tick() {
        if (this.minecraft.playerController.isInCreativeMode()) {
            this.minecraft.displayScreen(new CreativeScreen(this.minecraft.player));
        } else {
    ...
    protected void init() {
        if (this.minecraft.playerController.isInCreativeMode()) {
            ...
            String dropText = "Drop All";
            int margin = 15;
            this.addButton(new Button(width / 2 - ((margin + font.getStringWidth(dropText) + margin) / 2), guiTop - 25, (margin + font.getStringWidth(dropText) + margin), 20, new StringTextComponent(dropText), (button) -> dropItems()));
        }
    }
    ...
    private void dropItems() {
        if (minecraft.player != null && minecraft.playerController != null) {
            for (int i = 0; i < container.getInventory().size() && minecraft.currentScreen == this; ++i) {
