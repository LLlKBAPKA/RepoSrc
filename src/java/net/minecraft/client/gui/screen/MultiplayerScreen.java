...
import org.excellent.common.impl.proxy.GuiProxy;
import org.excellent.common.impl.proxy.ProxyConfig;
import org.excellent.common.impl.proxy.ProxyServer;
...
public class MultiplayerScreen extends Screen {
    ...
    protected void init() {
        ...
        this.addButton(new Button(this.width / 2 - 50, this.height - 52, 100, 20, new TranslationTextComponent("selectServer.direct"), (p_214286_1_) ->
        {
            ...
            this.minecraft.displayScreen(new ServerListScreen(this, this::func_214290_d, this.selectedServer));
        }));
        ...
        {
            this.selectedServer = new ServerData(I18n.format("selectServer.defaultName"), "", false);
            this.minecraft.displayScreen(new AddServerScreen(this, this::func_214284_c, this.selectedServer));
        }));
        this.btnEditServer = this.addButton(new Button(this.width / 2 - 154, this.height - 28, 70, 20, new TranslationTextComponent("selectServer.edit"), (p_214283_1_) ->
        {
            ...
            if (serverselectionlist$entry instanceof ServerSelectionList.NormalEntry entry) {

                ServerData serverdata = entry.getServerData();
                this.selectedServer = new ServerData(serverdata.serverName, serverdata.serverIP, false);
                this.selectedServer.copyFrom(serverdata);
                this.minecraft.displayScreen(new AddServerScreen(this, this::func_214292_b, this.selectedServer));
            }
        }));
        this.btnDeleteServer = this.addButton(new Button(this.width / 2 - 74, this.height - 28, 70, 20, new TranslationTextComponent("selectServer.delete"), (p_214294_1_) ->
        {
            ...
            if (serverselectionlist$entry instanceof ServerSelectionList.NormalEntry entry) {
                String s = entry.getServerData().serverName;
                ...
                if (s != null) {
                    ...
                    this.minecraft.displayScreen(new ConfirmScreen(this::func_214285_a, itextcomponent, itextcomponent1, itextcomponent2, itextcomponent3));
                }
            }
        }));
        ...
        String playerName = minecraft.getSession().getProfile().getName();
        if (!playerName.equals(ProxyConfig.lastPlayerName)) {
            ProxyConfig.lastPlayerName = playerName;
            if (ProxyConfig.accounts.containsKey(playerName)) {
                ProxyServer.proxy = ProxyConfig.accounts.get(playerName);
            } else {
                if (ProxyConfig.accounts.containsKey("")) {
                    ProxyServer.proxy = ProxyConfig.accounts.get("");
                }
            }
        }

        ProxyServer.proxyMenuButton = new Button(width - 125, 5, 120, 20, new StringTextComponent("Proxy: " + ProxyServer.getLastUsedProxyIp()), (buttonWidget) -> {
            minecraft.displayScreen(new GuiProxy(this));
        });
        this.addButton(ProxyServer.proxyMenuButton);

        this.addButton(ViaMCP.INSTANCE.getViaScreen());
    ...
    private void refreshServerList() {
        this.minecraft.displayScreen(new MultiplayerScreen(this.parentScreen));
    }
    ...
    private void connectToServer(ServerData server) {
        this.minecraft.displayScreen(new ConnectingScreen(this, this.minecraft, server));
    }
