...
import org.excellent.client.managers.module.impl.misc.Optimizer;
...
public class ParticleManager implements IFutureReloadListener {
    ...
    private <T extends IParticleData> void registerFactory(ParticleType<T> particleTypeIn, ParticleManager.IParticleMetaFactory<T> particleMetaFactoryIn) {
        ParticleManager.AnimatedSpriteImpl particlemanager$animatedspriteimpl = new AnimatedSpriteImpl();
    ...
    public CompletableFuture<Void> reload(IFutureReloadListener.IStage stage, IResourceManager resourceManager, Executor backgroundExecutor, Executor gameExecutor) {
        ...
        CompletableFuture<?>[] completablefuture = Registry.PARTICLE_TYPE.keySet().stream().map((p_lambda$reload$1_4_) ->
                CompletableFuture.runAsync(() -> this.loadTextureLists(resourceManager, p_lambda$reload$1_4_, map), backgroundExecutor)).toArray(CompletableFuture[]::new);
        return CompletableFuture.allOf(completablefuture).thenApplyAsync((p_lambda$reload$3_4_) ->
        {
            return this.atlas.stitch(resourceManager, map.values().stream().flatMap(Collection::stream), 0);
        ...
        try (
            ...
            if (list == null) {
                ...
                textures.put(particleId, list.stream().map((p_lambda$loadTextureLists$6_0_) ->
                        new ResourceLocation(p_lambda$loadTextureLists$6_0_.getNamespace(), "particle/" + p_lambda$loadTextureLists$6_0_.getPath())).collect(Collectors.toList()));
            }
    ...
    private void tickParticleList(Collection<Particle> particlesIn) {
        if (!particlesIn.isEmpty()) {
            ...
            if (j > 0) {
                ...
                for (Iterator<Particle> iterator1 = particlesIn.iterator(); iterator1.hasNext() && k > 0; --k) {
                    Particle particle1 = iterator1.next();
            ...
            if (iparticlerendertype != IParticleRenderType.NO_RENDER) {
                ...
                if (iterable != null) {
                    ...
                    for (Particle particle : iterable) {
                        if (!particle.shouldCull() || p_renderParticles_6_.isBoundingBoxInFrustum(particle.getBoundingBox())) {
                            try {
    ...
    public void addBlockDestroyEffects(BlockPos pos, BlockState state) {
        ...
        if (optimizer.isEnabled() && optimizer.checks().getValue("Ломание блоков")) {
            return;
        }
        boolean flag;
