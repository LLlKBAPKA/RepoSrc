...
public abstract class Particle {
    ...
    private final BlockPosM blockPosM = new BlockPosM();
    ...
    public void move(double x, double y, double z) {
        if (!this.collidedY) {
            ...
            if (this.canCollide && (x != 0.0D || y != 0.0D || z != 0.0D) && this.hasNearBlocks(x, y, z)) {
                Vector3d vector3d = Entity.collideBoundingBoxHeuristically(null, new Vector3d(x, y, z), this.getBoundingBox(), this.world, ISelectionContext.dummy(), new ReuseableStream<>(Stream.empty()), false, false);
                x = vector3d.x;
                y = vector3d.y;
                z = vector3d.z;
            }
