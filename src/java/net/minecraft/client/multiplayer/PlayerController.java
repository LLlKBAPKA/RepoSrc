...
import org.excellent.client.managers.module.impl.misc.HolyWorldAssist;
import org.excellent.client.managers.module.impl.player.FastBreak;
import org.excellent.client.managers.module.impl.player.Nuker;
...
public class PlayerController {
        ...
        if (this.mc.player.blockActionRestricted(this.mc.world, loc, this.currentGameType)) {
            ...
            if (this.currentGameType.isCreative()) {
                ...
                if (flag && blockstate1.getPlayerRelativeBlockHardness(this.mc.player, this.mc.player.world, loc) >= 1.0F) {
                    this.onPlayerDestroyBlock(loc);
                    if (Nuker.getInstance().isEnabled() && PlayerUtil.isFuntime()) Nuker.getInstance().updateNuker();
                } else {
                    ...
                    this.mc.world.sendBlockBreakProgress(this.mc.player.getEntityId(), this.currentBlock, (int) (curBlockDamageMP * 10.0F) - 1);
                }
            }
        ...
        if (this.blockHitDelay > 0) {
            ...
            if (blockstate.isAir()) {
                ...
                if (this.curBlockDamageMP >= 1.0F) {
                    ...
                    HolyWorldAssist holy = HolyWorldAssist.getInstance();
                    if (holy.isEnabled() && holy.addingBrokenBlocks().getValue())
                        IntStream.range(0, holy.brokenBlocks().getValue().intValue()).forEach(pos -> this.sendDiggingPacket(CPlayerDiggingPacket.Action.STOP_DESTROY_BLOCK, posBlock, directionFacing));
                    this.onPlayerDestroyBlock(posBlock);
                    this.curBlockDamageMP = 0.0F;
                    this.stepSoundTickCounter = 0.0F;
                    if (!FastBreak.getInstance().isEnabled()) this.blockHitDelay = 5;
                }
    ...
    public ClientPlayerEntity createPlayer(ClientWorld worldIn, StatisticsManager statsManager, ClientRecipeBook recipes) {
        return this.createPlayer(worldIn, statsManager, recipes, false, false);
    }

    public ClientPlayerEntity createPlayer(ClientWorld worldIn, StatisticsManager statsManager, ClientRecipeBook recipes, boolean sneakState, boolean sprintState) {
        return new ClientPlayerEntity(this.mc, worldIn, this.connection, statsManager, recipes, sneakState, sprintState);
    }
    ...
    public void attackEntity(PlayerEntity playerIn, Entity targetEntity) {
        ...
        event.hook();
        if (targetEntity.equals(playerIn) || event.isCancelled())
            return;
        this.syncCurrentPlayItem();
    ...
    public ItemStack windowClick(int windowId, int slotId, int mouseButton, ClickType type, PlayerEntity player) {
        short nextTransactionID = player.openContainer.getNextTransactionID(player.inventory);
        ItemStack itemstack = player.openContainer.slotClick(slotId, mouseButton, type, player);
        this.connection.sendPacket(new CClickWindowPacket(windowId, slotId, mouseButton, type, itemstack, nextTransactionID));
        return itemstack;
    }
    ...
    public void onStoppedUsingItem(PlayerEntity playerIn) {
        StopUseItemEvent event = new StopUseItemEvent();
        event.hook();
