...
import org.excellent.common.impl.datafix.SaturnTemperatureCache;
...
public final class Biome {
    ...
    public static final Codec<Biome> CODEC = RecordCodecBuilder.create((builder) ->
            builder.group(Climate.CODEC.forGetter((biome) -> biome.climate), Category.CODEC.fieldOf("category").forGetter((biome) -> biome.category), Codec.FLOAT.fieldOf("depth").forGetter((biome) -> biome.depth), Codec.FLOAT.fieldOf("scale").forGetter((biome) -> biome.scale), BiomeAmbience.CODEC.fieldOf("effects").forGetter((biome) -> biome.effects), BiomeGenerationSettings.CODEC.forGetter((biome) -> biome.biomeGenerationSettings), MobSpawnInfo.CODEC.forGetter((biome) -> biome.mobSpawnInfo)).apply(builder, Biome::new));
    public static final Codec<Biome> PACKET_CODEC = RecordCodecBuilder.create((builder) ->
            builder.group(Climate.CODEC.forGetter((biome) -> biome.climate), Category.CODEC.fieldOf("category").forGetter((biome) -> biome.category), Codec.FLOAT.fieldOf("depth").forGetter((biome) -> biome.depth), Codec.FLOAT.fieldOf("scale").forGetter((biome) -> biome.scale), BiomeAmbience.CODEC.fieldOf("effects").forGetter((biome) -> biome.effects)).apply(builder, (climate, category, depth, scale, ambience) -> new Biome(climate, category, depth, scale, ambience, BiomeGenerationSettings.DEFAULT_SETTINGS, MobSpawnInfo.EMPTY)));
    ...
    private final ThreadLocal<Long2FloatLinkedOpenHashMap> temperatureCache = SaturnTemperatureCache.saturnTemperatureCache;
    ...
    private float getTemperatureAtPosition(BlockPos pos) {
        ...
        if (pos.getY() > 64) {
            float f1 = (float) (TEMPERATURE_NOISE.noiseAt((float) pos.getX() / 8.0F, (float) pos.getZ() / 8.0F, false) * 4.0D);
    ...
    public boolean doesSnowGenerate(IWorldReader worldIn, BlockPos pos) {
        if (this.getTemperature(pos) >= 0.15F) {
            ...
            if (pos.getY() >= 0 && pos.getY() < 256 && worldIn.getLightFor(LightType.BLOCK, pos) < 10) {
                ...
                return blockstate.isAir() && Blocks.SNOW.getDefaultState().isValidPosition(worldIn, pos);
            }
    ...
    public static class Attributes {
        public static final Codec<Biome.Attributes> CODEC = RecordCodecBuilder.create((builder) ->
                builder.group(Codec.floatRange(-2.0F, 2.0F).fieldOf("temperature").forGetter((attributes) -> attributes.temperature), Codec.floatRange(-2.0F, 2.0F).fieldOf("humidity").forGetter((attributes) -> attributes.humidity), Codec.floatRange(-2.0F, 2.0F).fieldOf("altitude").forGetter((attributes) -> attributes.altitude), Codec.floatRange(-2.0F, 2.0F).fieldOf("weirdness").forGetter((attributes) -> attributes.weirdness), Codec.floatRange(0.0F, 1.0F).fieldOf("offset").forGetter((attributes) -> attributes.offset)).apply(builder, Attributes::new));
    ...
    public enum Category implements IStringSerializable {
        ...
        private static final Map<String, Biome.Category> BY_NAME = Arrays.stream(values()).collect(Collectors.toMap(Biome.Category::getName, (category) -> category));
    ...
    static class Climate {
        public static final MapCodec<Biome.Climate> CODEC = RecordCodecBuilder.mapCodec((builder) ->
                builder.group(RainType.CODEC.fieldOf("precipitation").forGetter((precipitation) -> precipitation.precipitation), Codec.FLOAT.fieldOf("temperature").forGetter((climate) -> climate.temperature), TemperatureModifier.CODEC.optionalFieldOf("temperature_modifier", TemperatureModifier.NONE).forGetter((climate) -> climate.temperatureModifier), Codec.FLOAT.fieldOf("downfall").forGetter((climate) -> climate.downfall)).apply(builder, Climate::new));
    ...
    public enum RainType implements IStringSerializable {
        ...
        private static final Map<String, Biome.RainType> BY_NAME = Arrays.stream(values()).collect(Collectors.toMap(Biome.RainType::getName, (precipitation) -> precipitation));
        private final String name;
    ...
    public enum TemperatureModifier implements IStringSerializable {
        ...
        private static final Map<String, Biome.TemperatureModifier> NAME_TO_MODIFIER_MAP = Arrays.stream(values()).collect(Collectors.toMap(Biome.TemperatureModifier::getName, (temperatureModifier) -> temperatureModifier));
