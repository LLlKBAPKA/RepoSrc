...
public abstract class World implements IWorld, AutoCloseable {
     ...
     * 16x128x16 field.
     */
    protected int updateLCG = (new FastRandom()).nextInt();
    ...
    protected World(ISpawnWorldInfo worldInfo, RegistryKey<World> dimension, final DimensionType dimensionType, boolean isRemote, boolean isDebug, long seed) {
    ...
    public boolean isBlockPresent(BlockPos pos) {
        return !isOutsideBuildHeight(pos) && this.getChunkProvider().chunkExists(pos.getX() >> 4, pos.getZ() >> 4);
    }
    ...
    public boolean isDirectionSolid(BlockPos pos, Entity entity, Direction direction) {
        if (isOutsideBuildHeight(pos)) {
            ...
            return ichunk != null && ichunk.getBlockState(pos).isTopSolid(this, pos, entity, direction);
        }
    }
    ...
    public void calculateInitialSkylight() {
        ...
        double d2 = 0.5D + 2.0D * MathHelper.clamp(MathHelper.cos(this.func_242415_f(1.0F) * ((float) Math.PI * 2F)), -0.25D, 0.25D);
