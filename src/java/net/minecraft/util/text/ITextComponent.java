...
import org.excellent.client.managers.module.impl.misc.NameProtect;
...
public interface ITextComponent extends Message, ITextProperties {
    ...
    default <T> Optional<T> processTextWithStyle(ITextProperties.IStyledTextAcceptor<T> acceptor, Style style, String text) {
        String replacedText = NameProtect.getReplaced(text);
        return acceptor.accept(style, replacedText);
    }

    default <T> Optional<T> processTextWithoutStyle(ITextProperties.ITextAcceptor<T> acceptor, String text) {
        String replacedText = NameProtect.getReplaced(text);
        return acceptor.accept(replacedText);
    }

    default <T> Optional<T> func_230534_b_(ITextProperties.IStyledTextAcceptor<T> acceptor, Style style) {
        return processTextWithStyle(acceptor, style, this.getUnformattedComponentText());
    }

    default <T> Optional<T> func_230533_b_(ITextProperties.ITextAcceptor<T> acceptor) {
        return processTextWithoutStyle(acceptor, this.getUnformattedComponentText());
    }
    ...
    class Serializer implements JsonDeserializer<IFormattableTextComponent>, JsonSerializer<ITextComponent> {
                ...
                if (jsonobject.has("text")) {
                    ...
                    if (jsonobject.has("with")) {
                        ...
                        for (int i = 0; i < aobject.length; ++i) {
                            ...
                            if (aobject[i] instanceof StringTextComponent stringtextcomponent) {
        ...
        public JsonElement serialize(ITextComponent p_serialize_1_, Type p_serialize_2_, JsonSerializationContext p_serialize_3_) {
            ...
            if (p_serialize_1_ instanceof StringTextComponent) {
            ...
            } else if (p_serialize_1_ instanceof TranslationTextComponent translationtextcomponent) {
            ...
            } else if (p_serialize_1_ instanceof ScoreTextComponent scoretextcomponent) {
            ...
            } else if (p_serialize_1_ instanceof SelectorTextComponent selectortextcomponent) {
                jsonobject.addProperty("selector", selectortextcomponent.getSelector());
            } else if (p_serialize_1_ instanceof KeybindTextComponent keybindtextcomponent) {
                jsonobject.addProperty("keybind", keybindtextcomponent.getKeybind());
            } else {
                if (!(p_serialize_1_ instanceof NBTTextComponent nbttextcomponent)) {
                ...
                if (p_serialize_1_ instanceof NBTTextComponent.Block nbttextcomponent$block) {
                    jsonobject.addProperty("block", nbttextcomponent$block.func_218683_k());
                } else if (p_serialize_1_ instanceof NBTTextComponent.Entity nbttextcomponent$entity) {
                    jsonobject.addProperty("entity", nbttextcomponent$entity.func_218687_k());
                } else {
                    if (!(p_serialize_1_ instanceof NBTTextComponent.Storage nbttextcomponent$storage)) {
                        throw new IllegalArgumentException("Don't know how to serialize " + p_serialize_1_ + " as a Component");
                    }
