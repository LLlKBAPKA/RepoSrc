...
import org.excellent.common.impl.viaversion.netty.handler.ViaDecoder;
import org.excellent.common.impl.viaversion.netty.handler.ViaEncoder;
...
public class NetworkManager extends SimpleChannelInboundHandler<IPacket<?>> {
    ...
    protected void channelRead0(ChannelHandlerContext context, IPacket<?> packet) {
        if (this.channel.isOpen()) {
            try {
                PacketEvent event = new PacketEvent(PacketEvent.Action.RECEIVE, packet);
                event.hook();
    ...
    public void sendPacket(IPacket<?> packet, @Nullable GenericFutureListener<? extends Future<? super Void>> futureListener) {
        PacketEvent event = new PacketEvent(PacketEvent.Action.SEND, packet);
        event.hook();
        ...
        if (this.isChannelOpen()) {
            ...
            this.dispatchPacket(packet, futureListener);
        } else {
            this.outboundPacketsQueue.add(new NetworkManager.QueuedPacket(packet, futureListener));
        }
    }
    ...
    public void sendPacketWithoutEvent(IPacket<?> packet, @Nullable GenericFutureListener<? extends Future<? super Void>> futureListener) {
        if (this.isChannelOpen()) {
            this.flushOutboundQueue();
            this.dispatchPacket(packet, futureListener);
        } else {
