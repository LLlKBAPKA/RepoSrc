...
import org.excellent.client.managers.module.impl.render.FullBright;
...
public class Shaders {
    ...
    private static final int baseAttribId = 11;
    public static int entityAttrib = baseAttribId;
    ...
    private static final int[] gbuffersFormat = new int[16];
    ...
    private static final int[] shadowBuffersFormat = new int[2];
    ...
    private static void reloadCustomTexturesLocation(ICustomTexture[] cts) {
        if (cts != null) {
            for (int i = 0; i < cts.length; ++i) {
                ...
                if (icustomtexture instanceof CustomTextureLocation customtexturelocation) {
    ...
    public static boolean isRenderBackFace(RenderType blockLayerIn) {
        if (blockLayerIn == RenderTypes.SOLID) {
            ...
            return blockLayerIn == RenderTypes.TRANSLUCENT && shaderPackBackFaceTranslucent.isTrue();
        }
    }
    ...
    private static int createVertShader(Program program, String filename) {
        ...
        if (inputstream == null) {
            ...
            if (i == 0) {
                ...
                ARBShaderObjects.glShaderSourceARB(i, s1);
                ARBShaderObjects.glCompileShaderARB(i);
    ...
    private static void setProgramUniforms(ProgramStage programStage) {
        ...
        setProgramUniform1f(uniform_screenBrightness, FullBright.getInstance().isEnabled() && FullBright.getInstance().mode().is("Гамма") ? 10 : (float) mc.gameSettings.gamma);
        ...
        if (clientworld != null) {
            ...
            if (entity != null) {
                ...
                if (entity instanceof LivingEntity livingentity) {
                    nightVision = 0.0F;
                    if (livingentity.isPotionActive(Effects.NIGHT_VISION) || (FullBright.getInstance().isEnabled() && FullBright.getInstance().mode().is("Ночное Зрение"))) {
    ...
    public static void setCameraShadow(MatrixStack matrixStack, ActiveRenderInfo activeRenderInfo, float partialTicks) {
        ...
        if (shadowMapIsOrtho) {
            GL43.glOrtho(-shadowMapHalfPlane, shadowMapHalfPlane, -shadowMapHalfPlane, shadowMapHalfPlane, 0.05F, 256.0D);
        } else {
    ...
    public static void preCelestialRotate(MatrixStack matrixStackIn) {
        matrixStackIn.rotate(Vector3f.ZP.rotationDegrees(sunPathRotation));
        ...
        if (dfb != null) {
            ...
            if (checkGLError("useComputeProgram") != 0) {
                ...
                if (vector3i == null) {
                    ...
                    int k = (int) Math.ceil((double) i / (double) vector3i1.getX());
                    int l = (int) Math.ceil((double) j / (double) vector3i1.getY());
