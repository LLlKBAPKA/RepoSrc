            ...
            if (!Config.isFromDefaultResourcePack(resourcelocation)) {
                ...
                if (!Config.isFromDefaultResourcePack(resourcelocation1)) {
                    ...
                    if (ibakedmodel != null && ibakedmodel != modelmanager.getMissingModel()) {
                        List list = ibakedmodel.getQuads(null, null, RANDOM);
                        ...
                        if (list.size() == 0) {
                            ...
                            for (BakedQuad bakedquad : (List<BakedQuad>) list) {
                                List list1 = ibakedmodel.getQuads(null, bakedquad.getFace(), RANDOM);
    ...
    private static IBakedModel getModelDoubleFace(IBakedModel model) {
        if (model == null) {
            ...
            for (int k = 0; k < adirection.length; ++k) {
                ...
                BakedQuad bakedquad1 = new BakedQuad(bakedquad.getVertexData().clone(), bakedquad.getTintIndex(), bakedquad.getFace(), bakedquad.getSprite(), bakedquad.applyDiffuseLighting());
